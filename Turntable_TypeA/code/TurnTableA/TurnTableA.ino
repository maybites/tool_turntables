...
void loop(){
  ...
  for(int i = 0; i < 3; i++){
    currentAnalogValue[i] = analogRead(potiAnalogPin[i]);
    if(currentAnalogValue[i] != lastAnalogValue[i]){
  ...
  if(!calibrated && digitalReadFast(signalPinZ)){
    ...
    if(flankCounter != newFlankCounter){
      ...
      if(flankCounter == 0 || flankCounter == 4096){
        myEnc.write(2048); 
        flankCounter = 2048;
      }
    }
...
void TX(){
    ...
    sendBuffer[0] = (calibrated)? 1: 0;
    sendBuffer[1] = map(lastAnalogValue[0], 0, 1023, 0, 254);
    sendBuffer[2] = map(lastAnalogValue[1], 0, 1023, 0, 254);
    sendBuffer[3] = map(lastAnalogValue[2], 0, 1023, 0, 254);
    sendBuffer[4] = turningDir + 127;
    sendBuffer[5] = value >> 7;
    sendBuffer[6] = value & 127;
    sendBuffer[7] = 255;
...
void TXHighRes(){
    ...
    //word value = flankCounter;
    sendBuffer[0] = (calibrated)? 1: 0;
    sendBuffer[1] = lastAnalogValue[0] >> 7;
    sendBuffer[2] = lastAnalogValue[0] & 127;
    sendBuffer[3] = lastAnalogValue[1] >> 7;
    sendBuffer[4] = lastAnalogValue[1] & 127;
    sendBuffer[5] = lastAnalogValue[2] >> 7;
    sendBuffer[6] = lastAnalogValue[2] & 127;
    sendBuffer[7] = value >> 7;
    sendBuffer[8] = value & 127;
    sendBuffer[9] = 255;
    Serial.write(sendBuffer, 10);  
}
